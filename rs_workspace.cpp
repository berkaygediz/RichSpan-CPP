...
void RS_Workspace::readSettings() {
    try {
        QSettings settings("berkaygediz", "RichSpan");
...
void RS_Workspace::initUI() {
    ...
    FallbackValues fallback;
    setWindowIcon(QIcon(":/resources/richspan_icon.ico"));
    ...
    connect(documentArea, &QTextEdit::textChanged, this, &RS_Workspace::updateStatistics);
    ...
    status_bar->showMessage(QString::number(starttime.msecsTo(endtime)) + " ms", 2500);
...
void RS_Workspace::updateTitle() {
    ...
    QString textMode = file.endsWith(".docx") ? translations[lang]["readonly"] : "";

    QString asterisk = textMode.isEmpty() && !isSaved ? "*" : "";

    setWindowTitle(file + asterisk + textMode + " — " + qApp->applicationDisplayName());
}

// void RS_Workspace::threadStart() {
//     if (!thread_running) {
//         richspan_thread->start();
//         thread_running = true;
//     }
// }
...
void RS_Workspace::toolbarTranslate() {
    ...
    QMap<QAction *, QPair<QString, QString>> actions
        = {{newaction, {"new", "new_message"}},
           ...
           {backgroundcolor, {"contentBackgroundColor", "contentBackgroundColor_message"}},
...
void RS_Workspace::updateToolbarLabel(QToolBar *toolbar, const QString &newLabel)
{
...
void RS_Workspace::LLMinitDock() {
    ...
    connect(predict_button, &QPushButton::clicked, this, &RS_Workspace::LLMpredict);
...
void RS_Workspace::LLMcontextPredict(bool action_type) {
    ...
    QString prompt;
    if (action_type) {
        prompt = QString("%1: %2").arg(action_type ? "Action" : "No Action", selected_text);
    } else {
...
QString RS_Workspace::LLMconvertCodeHTML(const QString &input_text) {
    ...
    QRegularExpression code_block_regex(R"(\[code\](.*?)\[/code\])",
                                        QRegularExpression::DotMatchesEverythingOption);

    auto replace_code_block = [](const QRegularExpressionMatch &match) -> QString {
        QString code_content = match.captured(1);
        return "<pre><code>" + code_content + "</code></pre>";
    };

    QRegularExpressionMatchIterator matchIterator = code_block_regex.globalMatch(modified_text);

    int offset = 0;
    while (matchIterator.hasNext()) {
        QRegularExpressionMatch match = matchIterator.next();
        QString replacement = replace_code_block(match);
        modified_text.replace(match.capturedStart(), match.capturedLength(), replacement);
    }
...
QString RS_Workspace::LLMescapeHTML(const QString &text) {
    ...
    QRegularExpressionMatchIterator matchIterator = html_escape_regex.globalMatch(modified_text);
...
void RS_Workspace::initActions() {
    ...
    QList<ActionDefinition> actionDefinitions = {
        {"newaction", "New", "Create a new file", [this]() { newFile(); }, QKeySequence::New},
        {"openaction", "Open", "Open an existing file", [this]() { openFile(); }, QKeySequence::Open},
        {"saveaction", "Save", "Save the current file", [this]() { saveFile(); }, QKeySequence::Save},
        {"saveasaction",
         "Save As",
        ...
        {"printaction",
         ...
         [this]() { printDocument(); },
         QKeySequence::Print},
        {"findaction", "Find", "Find text in the document", [this]() { find(); }, QKeySequence::Find},
        {"replaceaction",
         "Replace",
        ...
        {"undoaction",
         ...
         [this]() { documentArea->undo(); },
         QKeySequence::Undo},
        {"redoaction",
         "Redo",
         "Redo the last undone action",
         [this]() { documentArea->redo(); },
         QKeySequence::Redo},
        {"alignrightevent",
         "Align Right",
    ...
    for (const ActionDefinition &actionDef : actionDefinitions) {
        QAction *action = createAction(actionDef.text,
                                       actionDef.statusTip,
                                       actionDef.function,
                                       actionDef.shortcut);
...
void RS_Workspace::addImage() {
    ...
    if (!selectedFile.isEmpty()) {
        ...
        if (file.open(QIODevice::ReadOnly)) {
            ...
            QString imgTag = QString("<img src=\"data:%1;base64,%2\"/>").arg(mimeType, encodedData);
...
void RS_Workspace::find() {
    ...
    connect(findDialog, &QInputDialog::textValueSelected, this, &RS_Workspace::findText);
    findDialog->show();
}
...
void RS_Workspace::replace() {
    ...
    connect(replaceDialog, &QInputDialog::textValueSelected, this, &RS_Workspace::replaceText);
    replaceDialog->show();
}
...
void RS_Workspace::initToolbar() {
    ...
    file_toolbar->addActions(
        {createAction(
             "New", "Create a new file", [this]() { newFile(); }, QKeySequence::New),
         createAction(
             "Open", "Open an existing file", [this]() { openFile(); }, QKeySequence::Open),
         createAction(
             "Save", "Save the current file", [this]() { saveFile(); }, QKeySequence::Save),
         createAction(
             "Save As",
             "Save the file with a new name",
             [this]() { saveAs(); },
             QKeySequence::SaveAs),
         createAction(
             "Print", "Print the document", [this]() { printDocument(); }, QKeySequence::Print),
         createAction(
             "Undo", "Undo the last action", [this]() { /* Undo logic */ }, QKeySequence::Undo),
         createAction(
             "Redo",
             "Redo the last undone action",
             [this]() { /* Redo logic */ },
             QKeySequence::Redo),
         createAction(
             "Find", "Find text in the document", [this]() { find(); }, QKeySequence::Find),
         createAction(
             "Replace",
             "Replace text in the document",
             [this]() { replace(); },
    ...
    connect(powersaveraction, &QAction::toggled, this, &RS_Workspace::hybridSaver);
    ui_toolbar->addAction(powersaveraction);

    hide_ai_dock = createAction(
        "AI", "Show or hide the AI dock", [this]() { toggleDock(); }, QKeySequence("Ctrl+Shift+D"));
    ...
    font_toolbar->addActions({createAction(
                                  "Bold",
                                  "Make text bold",
                                  [this]() { contentBold(); },
    ...
    list_toolbar->addActions({createAction(
                                  "Bullet List",
                                  "Insert a bullet list",
                                  [this]() { bulletList(); },
                                  QKeySequence(Qt::CTRL + Qt::Key_B)),
    ...
    multimedia_toolbar->addActions({createAction(
        "Add Image", "Add an image", [this]() { addImage(); }, QKeySequence(Qt::CTRL + Qt::Key_X))});
}
